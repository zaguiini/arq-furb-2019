...
;1) PACMAN DEVE TER  SPLASH DE ABERTURA E AGUARDANDO PRESSIONAR UMA TECLA = 1,00 (X)
;2) PACMAN DEVE IR PARA TODOS OS LADOS COM ATALHOS = 1,00 (X)
;3) PACMAN DEVE SER BLOQUEADO PELA BARREIRA = 1,00 (X)
;4) PACMAN DEVE MUDAR FISIONOMIA = 1,00
;5) PACMAN DEVE TER BARREIRAS RANDOMICAS = 1,00 (X)
;6) PACMAN DEVE EMITIR AUDIO EM ALGUM MOMENTO = 1,00 (X)
;7) PACMAN DEVE MOSTRAR PONTUACAO NO CANTO INFERIOR = 1,00 (X)
;8) PACMAN DEVE APRESENTAR FRUTAS GRADATIVAMENTE = 1,00
;9) PACMAN DEVE FINALIZAR QUANDO PREENCHER COMPLETAMENTE DE FRUTAS = 1,00
;10) PACMAN DEVE FINALIZAR QUANDO COMER TODAS AS FRUTAS = 1,00 (X)
...
;-----------------------------------------------------
	...
	MOV ALEATORIO_DESEJADO, 111
	CALL INSERE_INICIAL
	
	MOV ALEATORIO_DESEJADO, 128
	CALL INSERE_INICIAL
	...
	FIM_DE_JOGO:
	   CALL GLCD_CLR
	   MOV BX, OFFSET MENSAGEM_FIM_DE_JOGO
	   MOV AL, 3
	   MOV AH, 2
	   CALL ESCREVE_LCD
	   MOV BX, OFFSET SCORE_MENSAGEM
...
INSERE_INICIAL:
   ...
   PUSH CX
   MOV CL, 5
REPETE_INSERE_INICIAL:
   CMP CL, 0
   JE SAI_INSERE_INICIAL
   CALL INSERE_ALEATORIO
   DEC CL
   JMP REPETE_INSERE_INICIAL
SAI_INSERE_INICIAL:
   POP CX
   POPF
   RET
...
INSERE_ALEATORIO:
   ...
   PUSH BX
   MOV AL, 0
REPETE_INSERE_ALEATORIO:
   CALL RANDOM
   CMP AL, 112
   JG REPETE_INSERE_ALEATORIO
   MOV BX, OFFSET MATRIZ_PACMANZERA
   ADD BL, AL
   PUSH AX
   MOV AL, 32
   CMP [BX], AL
   POP AX
   JNE REPETE_INSERE_ALEATORIO
   MOV AL, ALEATORIO_DESEJADO
   MOV [BX], AL
SAI_REPETE_INSERE_ALEATORIO:
   POP BX
   POP AX
   POPF
   RET
...
SAI_CONTA_PONTO:
   CALL EMITE_SOM_COMIDA
   DEC FRUTAS_REMANESCENTES
   POP BX
   POP AX
   POPF
   RET
...
ENABLE_PULSE:
   ...
   RET
	...
	;128 = BARREIRAZONA
	;111 = RANGAO VEIO
	MATRIZ_PACMANZERA  DB 129,32,32,32,32,32,32,32,32,32,32,32,32,32,32,32
...
DB 083H,083H,081H,081H,080H,080H,080H,000H,003H,00FH,0FCH,0E0H,000H,000H
	...
	SCORE DB "A0000"
	PONTO_ATUAL DB 0
	MENSAGEM_FIM_DE_JOGO DB "FIM DE JOGO!!", 0
